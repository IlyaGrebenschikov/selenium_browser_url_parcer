...
class GoogleSearchParser:
    ...
    def __init__(self, controller: BrowserControllerInterface) -> None:
        self._controller = controller

    def open(self) -> None:
        self._controller.get("https://www.google.com")

    def search(self, query: str, timeout: int) -> None:
        search_box = self._controller.wait_until(
            lambda d: d.find_element(*self.SEARCH_BOX), timeout
        )
        search_box.send_keys(query)
        search_box.send_keys(Keys.RETURN)
        self._controller.wait_until(
    ...
    def get_result_links(self, timeout: int) -> set[str]:
        ...
        links = self._controller.find_elements(By.XPATH, self.RESULT_LINKS_XPATH)
        return {
    ...
    def go_to_next_page(self, timeout: int) -> bool:
        try:
            ...
            next_button = self._controller.wait_until(
                lambda d: d.find_element(*self.NEXT_BUTTON), timeout
            )
            next_url = next_button.get_attribute("href")
            if next_url:
                self._controller.get(next_url)
                self._controller.wait_until(
                    lambda d: d.find_element(*self.SEARCH_RESULTS), timeout
                )
                return True
    ...
    def handle_consent_popup(self, timeout: int = 10) -> None:
        try:
            accept_button = self._controller.wait_until(
    ...
    def _close_location_popup(self, timeout: int = 15) -> None:
        try:
            ...
            popup_button = self._controller.wait_until(
